...
public class MeetingDAO {
	...
	public int findItId(Task task){
		...
		int id=0;
		String sql="select * from task1 where subject='"+task.getSubject()+"' and year='"+task.getYear()+"' and month='"+task.getMonth()+"'and day='"+task.getDay()+"'";
	...
	public List<Task> taskToday(String year,String month,String day){
		Connection conn = new ConnDB().getConnection();
		 String sql = "select * from task1 where year=? and month=? and day=?";  
	...
	public Task getById(int ID){
		...
		Connection	conn = new ConnDB().getConnection();
		int messageid=ID;
		String sql="select * from task1 where id=?";
		System.out.println("SQL:"+sql);
		try {
	...
	public List<Task> getTasks(String year,String month){
		Connection conn = new ConnDB().getConnection();
		 String sql = "select * from task1 where year=? and month=? order by id asc";   //����SQL���
	 ...
	 */
	 public int addTask(Task task){
		 ConnDB conn=new ConnDB();
		 int id=-1;
		 String sql="insert into task1(ID,year,month,day,start_time,end_time,state,subject,description,location)" +
	 ...
	 public boolean delTask(int taskId) {
	   ...
	   public boolean updateTask(Task task) {
		   Connection conn=new ConnDB().getConnection();
			String sql="update task1 set subject=?,description=?,start_time=?,end_time=?,location=?  where id=?" ;
